...
from traj.GPE.downtask import migrate_rank, migrate_prec, migrate_knn_self,migrate_mrr,infer_rank_split
...
def runtb_rank_knn_cs(dim,fun_sim_all=fun_base + fun_gpe, datasets=datasets, dorank=False, latex12=True):
...
def runtb_cross(dim,fun_sim_all=fun_base + fun_gpe, from_datas=datasets, dorank=False, latex12=True, metric='MeanRank'):
    ...
    for d1, from_data in enumerate(from_datas):
        for fi, method in enumerate(fun_sim_all):
            train_sim(method, from_data,dim)
...
def runtb_ana():
    ...
    data = runtb_rank_knn_cs(dimD,name_GPEs, datasets)
...
if __name__ == '__main__':
    ...
    init_cpu()
    init_gpu(dimD)
    runtb_rank_knn_cs(dimD)
    runtb_cross(dimD,metric='MRR')
