...
def infer_rank_split(method_name, from_data, to_data, ntr, nte):
    TA, TB = rank_split_data(to_data, nte)
    VA = infer_sim(method_name, from_data, to_data, TA)
    VB = infer_sim(method_name, from_data, to_data, TB)
    return (VA, VB)

@mcache(redir=out_dir(task_dir))
def migrate_rank(method_name, from_data, to_data, ntr, nte):
    VA, VB = infer_rank_split(method_name, from_data, to_data, ntr, nte)
    VA, VB = (VA.to(device), VB.to(device))
    mean_rank = rankvs(VA, VB).mean(dtype=float).item()
    return mean_rank

@mcache(redir=out_dir(task_dir))
def migrate_mrr(method_name, from_data, to_data, ntr, nte):
    VA, VB = infer_rank_split(method_name, from_data, to_data, ntr, nte)
    VA, VB = (VA.to(device), VB.to(device))
    ranks = rankvs(VA, VB)
...
@mcache(redir=out_dir(task_dir))
def migrate_prec(method_name, from_data, to_data, ntr, nte):
    VA, VB = infer_rank_split(method_name, from_data, to_data, ntr, nte)
    VA, VB = (VA.to(device), VB.to(device))
...
@mcache(redir=out_dir(task_dir))
def migrate_knn_self(method_name, from_data, to_data, ntr, nte, k):
    VQ1, VD1 = infer_rank_split(method_name, from_data, to_data, ntr, nte)
    VQ1, VD1 = (VQ1.to(device), VD1.to(device))
    K1 = knnvd(VQ1, VD1, k)
    ts = load_traj(to_data, nte)[1]
    VT = infer_sim(method_name, from_data, to_data, ts)
    K2 = knnvd(VT, VT, k)
