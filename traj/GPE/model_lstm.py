...
def _lstm_model_path(method_name: str, train_city: str,dim):
    ...
    path = os.path.join(out_dir('ckpt_lstm'), f'{method_name}_{train_city}_{lld.nTrain}-{lld.nTest}_d{dim}n{num_layer}b{(1 if bidir else 0)}_d{augment_drop_rate}n{augment_noise_size}_l{ts_max_len}{ts_min_len}b{batch_size}e{epoch_min}s{earlystop}.th')
    if os.path.exists(path): load_weight(model, path)
    return (model, path)

def train_lstm(method_name: str, train_city: str,dim,if_exist=False):
    model, path = _lstm_model_path(method_name, train_city,dim)
...
def infer_lstm(method_name, train_city, to_city, TS, dim,do_noise=False) -> Tensor:
