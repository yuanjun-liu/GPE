...
import numpy as np
from traj.exp_base.GPE_bases import load_gpe_others
...
class BaseSimModel(nn.Module):
    def __init__(self, ebd: nn.Module, ename, dim_ebd, dim_hid, dim_out, num_layer=2, bidir=True, device=auto_device()) -> None:
        ...
        self.ebd = nn.Sequential(ebd, _lin_net(_num_nn(ename), dim_ebd, dim_hid))
        self.dim_hid, self.bidir, self.num_layer, self.device = (dim_hid, bidir, num_layer, device)
        self.lstm = nn.LSTM(dim_hid, dim_hid, num_layer, bidirectional=bidir)
...
def name2ebd(method_name: str, city: str,dim) -> nn.Module:
    ...
    if 'bGPE' in method_name:
        return load_gpe_others(method_name,**kw)
    raise RuntimeError('bad ebd name in model_bs.py')

def _sim_model_path(method_name: str, train_city: str,dim1,dim2):
    ebd: nn.Module = name2ebd(method_name, train_city,dim1)
    model = BaseSimModel(ebd, method_name, dim1, dim2, dim2, num_layer=num_layer, bidir=bidir)
    path = os.path.join(out_dir('ckpt_bs'), f'{method_name}_{train_city}_{lld.nTrain}-{lld.nTest}_d{dim1}n{num_layer}b{(1 if bidir else 0)}_d{augment_drop_rate}n{augment_noise_size}_l{ts_max_len}{ts_min_len}b{batch_size}e{epoch_min}s{earlystop}.th')
    if os.path.exists(path): load_weight(model, path)
    return (model, path)

def train_bs(method_name: str, train_city: str,dim1,dim2):
    model, path = _sim_model_path(method_name, train_city,dim1,dim2)
    if os.path.exists(path): return True
    print.set(method_name + str(dim1)+'/' + train_city + '-' + str(lld.nTrain), newfile=True)
    print('train_method', method_name, dim1,dim2, train_city, lld.nTrain)
...
def infer_bs(method_name, train_city, to_city, TS, dim1,dim2,do_noise=False):
