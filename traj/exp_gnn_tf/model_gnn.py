...
device = auto_device()
num_layer = 2 # modify in BaseGnnModel
bidir = False
epoch_max = 20
epoch_min = 2
earlystop = 100
...
edges_all=torch.tensor(sum([[[i,i+1],[i+1,i]] for i in range(ts_max_len-1)],[]),dtype=torch.long).t().contiguous()
...
class BaseTrajDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        tr2 = torch.zeros((ts_max_len, self.dim))
        tr2[:le2] = t2
        return (tr, le, tr2, le2)

def ts2gs(Ts,Ls):
    xs=[Ts[i][:Ls[i]] for i in range(len(Ls))]
    edges=[edges_all[:Ls[i]*2-2].clone() for i in range(len(Ls))]
    batch_idx=[torch.zeros(Ls[i],dtype=int)+i for i in range(len(Ls))]
    xs=torch.concat(xs,dim=0) ; edges=torch.concat(edges,dim=-1) ; batch_idx=torch.concat(batch_idx)
    return xs,edges,batch_idx
...
class BaseGnnModel(nn.Module):
    def __init__(self, ebd: nn.Module, ename, dim_ebd, dim_hid, dim_out, num_layer=2, bidir=True, device=auto_device()) -> None:
        ...
        self.g1=GATConv(dim_ebd,dim_hid)
        self.relu=nn.ReLU()
        self.g2=GATConv(dim_hid,dim_hid) # 2 layers
    ...
    def forward(self,T,edge,batch):
        x=self.ebd(T) # len*bs*dim

        x=self.g1(x,edge) 
        x=self.relu(x)
        x=self.g2(x,edge)
        x=scatter(x, batch, dim=0, reduce='mean') # bs*dim
...
def _gnn_model_path(method_name: str, train_city: str,dim):
    ebd: nn.Module = name2ebd(method_name, train_city,dim)
    model = BaseGnnModel(ebd, method_name, dim, dim, dim, num_layer=num_layer, bidir=bidir)
    path = os.path.join(out_dir('ckpt_gnn'), f'{method_name}_{train_city}_{lld.nTrain}-{lld.nTest}_d{dim}n{num_layer}b{(1 if bidir else 0)}_d{augment_drop_rate}n{augment_noise_size}_l{ts_max_len}{ts_min_len}b{batch_size}e{epoch_min}s{earlystop}.th')
    if os.path.exists(path): load_weight(model, path)
    return (model, path)

def train_gnn(method_name: str, train_city: str,dim,test_exist=False):
    model, path = _gnn_model_path(method_name, train_city,dim)
    ...
    for ep in range(epoch_max):
        for ts, les,  ts_p, les_p in dataloder:
            x,edge,bidx=ts2gs(ts,les)
            x_p,edge_p,bidx_p=ts2gs(ts_p,les_p)
            [x,edge,bidx,x_p,edge_p,bidx_p]=[item.to(device) for item in [x,edge,bidx,x_p,edge_p,bidx_p]]
            vs, vs_pos = (model(x,edge,bidx), model(x_p,edge_p,bidx_p))
...
def infer_gnn(method_name, train_city, to_city, TS, dim,do_noise=False) -> Tensor:
    ...
    res = []
    with torch.no_grad():
        for ts, les in DataLoader(BaseTrajDataset(to_city, TS, train=False, do_noise=do_noise), batch_size=batch_size, shuffle=False, drop_last=False):
            x,edge,bidx=ts2gs(ts,les)
            x,edge,bidx=(x.to(device),edge.to(device),bidx.to(device))
            vs = model(x,edge,bidx).detach().cpu()
            res.append(vs)
    model.cpu()
