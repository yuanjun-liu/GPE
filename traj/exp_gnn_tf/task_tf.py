...
from traj.exp_gnn_tf.model_tf import infer_tf
...
def infer_rank_split(method_name, from_data, to_data, dim,ntr, nte):
    ...
    VA = infer_tf(method_name, from_data, to_data, TA,dim)
    VB = infer_tf(method_name, from_data, to_data, TB,dim)
    return (VA, VB)
...
def migrate_knn_self(method_name, from_data, to_data, dim,ntr, nte, k):
    ...
    VT = infer_tf(method_name, from_data, to_data, ts,dim)
    K2 = knnvd(VT, VT, k)
